...
CREATE TABLE activities (
  ...
  activity_time TIME,
  location TEXT,
  activity_type TEXT CHECK (activity_type IN ('transportation', 'accommodation', 'activity', 'food')) NOT NULL,
  note TEXT,
...
-- Enable RLS for settlements table
...
-- Enable RLS for gallery_images table
...
-- Enable RLS for image_comments table
...
-- Enable RLS for idea_votes table
...
-- Enable RLS for idea_comments table
...
DROP POLICY IF EXISTS "Users can upload files for their trips" ON storage.objects;
DROP POLICY IF EXISTS "Users can view files for their trips" ON storage.objects;
DROP POLICY IF EXISTS "Users can update files for their trips" ON storage.objects;
DROP POLICY IF EXISTS "Users can delete files for their trips" ON storage.objects;
